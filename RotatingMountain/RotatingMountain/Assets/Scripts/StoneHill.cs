...
public class StoneHill : MonoBehaviour
{
    ...
    void Update()
    {
        if(Vector3.Distance(this.transform.position,player.transform.position)<=3 && CouldInteract)
        {
            ...
            {
                CouldInteract = false;
                print("Player interacting with StoneHill");
                PlayerAttributes.Instance.HandleAttributesValueChange(PlayerAttributes.ATTRIBUTES_TYPE.PHSICAL_POWER, 10f);
                PlayerAttributes.Instance.HandleAttributesValueChange(PlayerAttributes.ATTRIBUTES_TYPE.SAN, -5f);
