...
public class Stone : MonoBehaviour
{
    ...
    void Update()
    {
        if(Vector3.Distance(this.transform.position,player.transform.position)<=3 && CouldInteract)
        {
            ...
            {
                CouldInteract = false;
                print("Player interacting with Stone");
                GameManager.Instance.HandleTimerChange(-20f);
                PlayerAttributes.Instance.HandleAttributesValueChange(PlayerAttributes.ATTRIBUTES_TYPE.PHSICAL_POWER, 30f);
                ...
                PlayerAttributes.Instance.HandleAnimationStateChange(PlayerAttributes.INTERACTION_ANIMATION_TYPE.REST);
            }
        }
        else
        {
